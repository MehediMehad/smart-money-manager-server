...
import { formatDate, getDayRange } from '../../utils/date';
...
const groupAmountByDate = <T extends { amount: number; date: Date }>(items: T[]) => {
...
const getDashboardOverview = async (userId: string, monthQuery?: string) => {
  ...
  const goals = savingsGoals.map((goal) => ({
    ...
    percent: goal.targetAmount > 0 ? Math.round((goal.savedAmount / goal.targetAmount) * 100) : 0,
  }));

  const buildBudget = (title: string, budgetAmount: number, spentAmount: number, color: string) => {
    ...
    return {
      ...
      budget: budgetAmount,
      spent: spentAmount,
      percent: budgetAmount > 0 ? Math.round((spentAmount / budgetAmount) * 100) : 0,
      left,
      color,
    };
  };
